...
static
integer
MIX_MAX_FORMULA_QTY = 100 // 每个
目标物品
最大支持的公式数量
private
static
integer
MIX_MAX_FLAG_QTY = 100 // 每条
公式
最大支持的零件种类数量
private
static
integer
hk_mix_flag_qty = MIX_MAX_FORMULA_QTY - 1
...
integer
local integer
qty = LoadInteger(hash_item_mix, 0, StringHash("_TOTAL_QTY_"))
if (qty <= 0) then
    set
    qty = 0
    endif
    ...
    nothing
    call
    SaveInteger(hash_item_mix, 0, StringHash("_TOTAL_QTY_"), qty)
    endmethod
    ...
    integer
    local integer
    qty = LoadInteger(hash_item_mix, itemId, StringHash("_FORMULA_QTY_"))
    if (qty <= 0) then
        set
        qty = 0
        endif
        ...
        nothing
        call
        SaveInteger(hash_item_mix, itemId, StringHash("_FORMULA_QTY_"), qty)
        endmethod
        ...
        integer
        local integer
        qty = LoadInteger(hash_item_mix, itemId, StringHash("_FORMULA_RESULT_QTY_" + I2S(x)))
        if (qty <= 0) then
            set
            qty = 0
            endif
            ...
            takes
            integer
            itemId, integer x, integer
            qty
            returns
            nothing
            call
            SaveInteger(hash_item_mix, itemId, StringHash("_FORMULA_RESULT_QTY_" + I2S(x)), qty)
            endmethod
            ...
            integer
            local integer
            qty = LoadInteger(hash_item_mix, itemId, StringHash("_FORMULA_FLAGMENT_QTY_" + I2S(x)))
            if (qty <= 0) then
                set
                qty = 0
                endif
                ...
                nothing
                call
                SaveInteger(hash_item_mix, itemId, StringHash("_FORMULA_FLAGMENT_QTY_" + I2S(x)), qty)
                endmethod
                ...
                integer
                itemId, integer x, integer
                y, integer flagmentType
                returns
                nothing
                call
                SaveInteger(hash_item_mix, itemId, StringHash("_FORMULA_FLAGMENT_NEED_TYPE_" + I2S(x) + "_" + I2S(y)), flagmentType)
                endmethod
                ...
                integer
                local integer
                qty = LoadInteger(hash_item_mix, itemId, StringHash("_FORMULA_FLAGMENT_NEED_QTY_" + I2S(x) + "_" + I2S(y)))
                if (qty <= 0) then
                    set
                    qty = 0
                    endif
                    ...
                    nothing
                    call
                    SaveInteger(hash_item_mix, itemId, StringHash("_FORMULA_FLAGMENT_NEED_QTY_" + I2S(x) + "_" + I2S(y)), qty)
                    endmethod
                    ...
                    integer
                    local integer
                    qty = LoadInteger(hash_item_mix, itemId, StringHash("_FLAGMENT_QTY_"))
                    if (qty <= 0) then
                        set
                        qty = 0
                        endif
                        ...
                        nothing
                        call
                        SaveInteger(hash_item_mix, itemId, StringHash("_FLAGMENT_QTY_"), qty)
                        endmethod
                        ...
                        nothing
                        call
                        SaveBoolean(hash_item_mix, itemId, StringHash("_FLAGMENT_ISSET_" + I2S(thistype.getTotalQty())), true)
                        endmethod
                        ...
                        nothing
                        call
                        SaveInteger(hash_item_mix, itemId, StringHash("_FLAGMENT_RESULT_" + I2S(x)), result)
                        endmethod
                        ...
                        takes
                        integer
                        flagmentId, integer x, integer
                        index
                        returns
                        nothing
                        call
                        SaveInteger(hash_item_mix, flagmentId, StringHash("_FLAGMENT_FORMULA_INDEX_" + I2S(x)), index)
                        endmethod
                        ...
                        integer
                        local integer
                        qty = LoadInteger(hash_item_mix, itemId, StringHash("_FLAGMENT_NEED_QTY_" + I2S(x)))
                        if (qty <= 0) then
                            set
                            qty = 0
                            endif
                            ...
                            nothing
                            call
                            SaveInteger(hash_item_mix, itemId, StringHash("_FLAGMENT_NEED_QTY_" + I2S(x)), qty)
                            endmethod
                        ...
                        */
                        ...
                        if (getFlagmentIsset(flagmentItemId) ! = true)then
                        // 判断是否新公式
                        //新零件, 零件服务公式数量+1
