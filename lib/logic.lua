...
--极坐标位移
hlogic.polarProjection = function(x, y, dist, angle)
    return {
...
--获取物品几率叠加几率
--根据固定因子及增益因子计算几率因子
hlogic.oddsItem = function(odds_stable, odds_gain, timers)
    return odds_stable + odds_stable * ((timers - 1) * odds_gain)
end

--计算属性特效效果叠加
hlogic.coverAttrEffectVal = function(value1, value2)
...
-- 四舍五入
hlogic.round = function(decimal)
    return math.floor((decimal * 100) + 0.5) * 0.01
end

--数字格式化
hlogic.numberFormat = function(value)
    local txt = ""
...
hlogic.getDegBetweenUnit = function(u1, u2)
...
--获取两个单位距离
hlogic.getDistanceBetweenUnit = function(u1, u2)
